...
NSString * BeInstanceOf::failure_message_for(const U & value) const {
    ...
    id idValue = value;
    if ([NSStringFromClass(expectedClass_) isEqualToString:NSStringFromClass([idValue class])]) {
...
bool BeInstanceOf::matches(const U & actualValue) const {
    id idValue = actualValue;
    if (includeSubclasses_) {
        return [idValue isKindOfClass:expectedClass_];
    } else {
        return [idValue isMemberOfClass:expectedClass_];
    }
}
}}}
